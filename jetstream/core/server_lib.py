...
class LLMUtilities(jetstream_pb2_grpc.UtilitiesServicer):
  ...
  def model_warmup(self):
    try: 
      self._driver.warmup_enabled = aot_utils.layout_params_and_compile_executables(
  ...
  async def ModelWarmup(
    ...
    if request.enable is False:
      ...
      return jetstream_pb2.ModelWarmupResponse(warmup_enabled=self._driver.warmup_enabled)
